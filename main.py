...
# เพิ่มพาธของโฟลเดอร์ปัจจุบันไปยัง sys.path เพื่อให้สามารถ import โมดูลอื่น ๆ ได้
script_dir = Path(__file__).resolve().parent
sys.path.append(str(script_dir))

from live_mode import run_live_mode_for_user
from trial_mode import start_trial_mode as run_trial_mode # หากยังคงใช้ trial mode
from utils import connect_gsheet # สำหรับการโหลดข้อมูลผู้ใช้จาก Google Sheet
...
# --- ฟังก์ชันสำหรับโหลดข้อมูลผู้ใช้และสิทธิ์จาก Google Sheet (ย้ายมาจาก auth.py เดิม) ---
...
def run_cli_mode(all_configs, gsheet_users_data):
    ...
    if user_auth_data:
        print(f"✅ Welcome, {username}! (Role: {user_auth_data['role']})")
        ...
        if user_auth_data['role'] != 'admin' and len(available_profiles_for_user) > user_auth_data['max_profiles']:
            ...
            print(f"ℹ️ คุณสามารถเข้าถึงได้สูงสุด {user_auth_data['max_profiles']} โปรไฟล์.")

        if not available_profiles_for_user:
            print("❌ ไม่พบโปรไฟล์ที่ใช้งานได้สำหรับ Username นี้ใน user_config.json")
            return
        ...
        try:
            ...
            while True:
                try:
                    branch_choice = int(input("กรุณาเลือกหมายเลขสาขา: "))
            ...
            while True:
                try:
                    time_choice = int(input("กรุณาเลือกหมายเลขเวลา: "))
